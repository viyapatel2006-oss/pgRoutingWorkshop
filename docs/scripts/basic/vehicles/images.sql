...
-- Penalizing with 3 times the costs of residential roads
...
SELECT * FROM pgr_dijkstra(
  ...
  USING (tag_id) ',
ARRAY[@ID_1@,@ID_2@,@ID_3@,@ID_4@, @ID_3@],
ARRAY[@ID_1@,@ID_2@,@ID_3@,@ID_4@, @ID_3@])
)
SELECT seq,
  ...
  WHEN start_vid = @ID_3@ THEN '@PLACE_5@' END
  ||','||
  ...
  WHEN end_vid = @ID_3@ THEN '@PLACE_5@' END
  AS name,
